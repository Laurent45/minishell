...
/*   Updated: 2022/07/22 16:29:30 by lfrederi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_tokens(t_list *tokens)
{
	for (; tokens; tokens = tokens->next)
		printf("code = %d et word = %s\n", ((t_token *) tokens->content)->code, ((t_token *) tokens->content)->str);
}
...
void	print_command(t_list *commands)
{
	for (; commands; commands = commands->next)
	{
		...
		for (t_list *tmp = cmd->redir; tmp; tmp = tmp->next)
		{
			t_redir *redir = (t_redir *) tmp->content;
			printf("redir code => %d\n", redir->code);
			if (redir->file)
				printf("file => %s\n", redir->file);
			if (redir->heredoc)
